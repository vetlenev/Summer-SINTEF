...
seed = rng();
seed.Seed = 2032; % Must set here, otherwise not updated
...
for i=anticline_idx
    ...
    z_anticline_start = -r*sin(theta) + z_start(i) + r*cos(pi/2-theta0); % -r*sin to get anticline (since z positive downwards)
    z_anticline_stop = -r*sin(theta) + z_stop(i) + r*cos(pi/2-theta0); % + r*cos(pi/4) to shift cap to bottom of spherical cap
...
dt = rampupTimesteps(6000*day(), 25*day(), 8);
n_steps = numel(dt);
...
%     drawnow, pause(0.2)
    ...
    %[tot_vol, lowperm_vol] = UtilFunctions.Co2VolumeRatio(dims, gridsize, all_lowperm_cells, 1-S);
    [tot_vol, top_vol] = UtilFunctions.Co2VolumeRatio(G, top_settled_cells, 1-S, rock, fluid);
...
used_lab = {};
f3 = figure(3);
% only one plot per unique num layer
fi = 3;
...
xlabel('Time step');
title('Ratio of total CO2 volume residing in interior');
drawnow
