%% 2D CO2 migration - no leakage, steady-state solution
...
seed = rng();
seed.Seed = 6457; % Must set here, otherwise not updated
...
plot_base_dir = strcat(ROOTDIR, '../summer_sintef/case3/plots_shutoff_noleakage');
data_dir = strcat(ROOTDIR, '../summer_sintef/case3/data_shutoff_noleakage');
...
pz = fluid.rhoWS * norm(gravity) * uniquetol(z(interior_cells)); % hydrostatic pressure in low-perm region
bc = addBC(bc, interior_east_edges, 'pressure', pz, 'sat', [1 0]);

tot_time = 22000*day();
dt = rampupTimesteps(tot_time/2, 75*day(), 10);
n_dt_prev = numel(dt);
new_dt = 2*dt(n_dt_prev);
last_dts = (tot_time/2)/new_dt;
dt(n_dt_prev+1:n_dt_prev+1+last_dts) = new_dt;
n_steps = numel(dt);

schedule = simpleSchedule(dt, 'W', W, 'bc', bc);

%% Shut off well just before halfway
inj_stop = fix(0.2*n_steps);
...
residual_ratios = ones(numel(states)+1, 1);
...
for i=1:numel(states)
    ...
    if i > 0 % no injected vol at first step --> residual ratio make no sense!
